...
Northstar is a Windows browser shell over the Microsoft Evergreen WebView2 runtime. It does not claim to maintain an independent web engine.

| Layer | Implementation | Ownership and update path |
| --- | --- | --- |
| Browser foundation | Northstar WPF shell plus WebView2 | Northstar shell; Microsoft runtime |
...
- Verify a renderer crash does not terminate the Northstar shell.
...
Site isolation must be validated on the shipped Windows runtime. Northstar will not make an absolute site-isolation claim based only on Chromium ancestry because runtime policies and implementation details can change.
