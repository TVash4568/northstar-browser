...
Northstar is not currently an enterprise-managed browser. WebView2 provides a maintained Windows engine, but it does not automatically give the Northstar shell Microsoft Edge's management, identity or compliance estate.
...
| Active Directory | No Northstar-specific integration | Managed deployment, authentication design and domain-environment testing |
...
| Microsoft 365 | Ordinary website compatibility only | No Northstar-native integration is claimed |
...
| Stable channel | Evergreen WebView2 engine plus alpha Northstar shell | Signed Northstar releases, staged rollout, rollback and supported release lifecycle |
...
Northstar must not reuse Microsoft Edge enterprise claims merely because both use WebView2/Chromium components. Enterprise controls belong to the complete browser product and its management infrastructure.
